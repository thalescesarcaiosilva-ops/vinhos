...
for (const s of scripts.slice(0, 5)) {
  const url = s.startsWith("http") ? s : `https://vinellevinhos.vercel.app${s}`;
  const js = await fetch(url);
  const text = await js.text();
  const hasSupabase = text.includes("zsfhnjrotkbzyikkxmnm");
  ...
  if (hasSupabase) {
    const m = text.match(/zsfhnjrotkbzyikkxmnm\.supabase\.co/g);
